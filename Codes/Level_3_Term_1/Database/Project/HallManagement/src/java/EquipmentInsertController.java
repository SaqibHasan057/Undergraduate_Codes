...
public class EquipmentInsertController extends HttpServlet{
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if(roomID.trim().length()==0 || cost.trim().length()==0 )
        {
            ...
            db.addNewEquipment(Integer.parseInt(cost), roomID, name);
            System.out.println("BUET");
            RequestDispatcher rd = request.getRequestDispatcher("addEquipment.jsp");
            rd.forward(request, response);
        }   
    }
