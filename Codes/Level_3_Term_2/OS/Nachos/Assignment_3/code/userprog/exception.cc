...
ExceptionHandler(ExceptionType which)
{
    ...
    if (which == SyscallException) {
    	if(type == SC_Halt){
			...
			space->localExecutable=executable;
			Thread * t = new Thread("tname");
			t->space = space;
			t->space->localExecutable = executable;
    ...
    {
    	printf("We have encountered a PageFaultException!!\n");

    	int addr = machine->ReadRegister(39);
    	currentThread->space->loadIntoFreePage(addr);

    	printf("PageFaultException has been resolved!!\n\n\n");
